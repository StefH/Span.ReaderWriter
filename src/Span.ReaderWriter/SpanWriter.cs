...
public ref struct SpanWriter
{
    ...
    public int Write(Guid value, int? position = null)
    {
        // See https://www.youtube.com/watch?v=B2yOjLyEZk0
        Span<byte> idBytes = stackalloc byte[Constants.SizeOfGuid];

        MemoryMarshal.Write(idBytes, ref value);

        idBytes.CopyTo(Span.Slice(position ?? Position));

        return UpdatePosition(Constants.SizeOfGuid, position ?? Position);
    }
